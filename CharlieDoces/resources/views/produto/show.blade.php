...
<main class="detalhesProduto">
    ...
    <section class="detalhesProdutoInfo">
        ...
        <p>Categoria: {{ $produto->Categoria ? $produto->Categoria->CATEGORIA_NOME : 'Categoria não disponível' }}</p>
        ...
        <button class="btn-AddCarrinho" type="button"><a href="/carrinho/{{$produto->PRODUTO_ID}}">Adicionar ao carrinho</a></button>
    </section>
            ...
            <div class="carousel-cards-inner">
                @foreach($produtosRelacionados as $produtoRelacionado)
                    <section class="cardProduto">
                        <div class="carousel">
                            <div class="carousel-images">
                                @if($produtoRelacionado->Produto_imagem->isNotEmpty())
                                    @foreach($produtoRelacionado->Produto_imagem as $imagem)
                                        <img src="{{ $imagem->IMAGEM_URL }}" alt="Imagem do Produto">
                                    @endforeach
                                @else
                                    <p>Nenhuma imagem disponível.</p>
                        ...
                        <a class="nomeProduto" href="/produto/{{ $produtoRelacionado->PRODUTO_ID }}">{{ $produtoRelacionado->PRODUTO_NOME }}</a>
                        <div class="orientacaoPreco">
                            <span class="precoProduto">
                                <span class="precoFinal">R${{ $produtoRelacionado->PRODUTO_PRECO - $produtoRelacionado->PRODUTO_DESCONTO }}</span>
                            </span>
                        </div>
                    </section>
                @endforeach
